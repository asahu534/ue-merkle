...
function createSlide(row, slideIndex, carouselId) {
  ...
  // Group consecutive CTA link paragraphs (a single link per paragraph) into an
  // actions container so the buttons render as a row, matching the source teaser.
  const content = slide.querySelector('.carousel-hero-slide-content');
  if (content) {
    const isButtonPara = (el) => el
      && el.tagName === 'P'
    ...
    [...content.children].forEach((child) => {
      if (isButtonPara(child)) {
        if (!actions) {
          actions = document.createElement('div');
          actions.classList.add('carousel-hero-slide-actions');
          content.insertBefore(actions, child);
        }
        actions.append(child.firstElementChild);
        child.remove();
      } else {
        actions = null;
      }
