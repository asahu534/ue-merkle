...
- **Trainees must not use client content.** All exercises use a made-up brand, **"Summit Outdoor Co."** (outdoor gear and trip guides). The Merkle repo is kept for trainers as a **reference implementation and answer key** (see §11).
...
- [ ] Save this document as `docs/training/eds-upskilling-assignment.md`, outside served paths, and add `docs/` to `.hlxignore` (requires Execute mode)
...
- [ ] Share this assignment (without §15) with the training team
...
## Notes
- **This is still a plan.** Plan mode is on, so I haven't written any files. Switch to Execute mode and I'll save it under `docs/training/`. It goes outside served paths, with `docs/` added to `.hlxignore` so it isn't published on the site. As you prefer, I won't commit or push it.
- **Optional PDF plugin.** There's a project-management plugin that can generate a PDF of this assignment. It isn't enabled yet. If you want it, tell me and I'll enable it in `.agents/settings.json`.
- **Section 15 is trainer-only.** Share the document without §15 (the answer key).
- **Assumptions you can change:** 6 weeks, a fictional brand, and three repos (one per content source). Tell me if you want a shorter bootcamp, a single-track version, or a different scenario.
