...
- **Trainees must not use client content.** All exercises use a made-up brand, **"Summit Outdoor Co."** (outdoor gear and trip guides). Never copy content, branding or code from client projects.
...
| # | Symptom | Your root cause (fill in) |
|---|---|---|
| 1 | UE: `Cannot read properties of undefined (reading 'modelId')` |  |
| 2 | Listing shows "No pages found" on author but works on `.aem.page` |  |
| 3 | Facet panel is empty |  |
| 4 | "Load more" visible even with 3 results |  |
| 5 | Third-party API fails only in production |  |
| 6 | Lint fails: too many cells |  |
| 7 | Layout jumps when fonts load |  |
| 8 | UE fields stop being editable after the block decorates |  |
| 9 | Stylelint `no-descending-specificity` error |  |
| 10 | Screenshot shows a blank page in tests |  |
